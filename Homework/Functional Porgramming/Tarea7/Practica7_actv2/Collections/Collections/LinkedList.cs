...
namespace Collections
{
    ...
    static class Extensors {
        ...
        public static T Search<T> (this LinkedList<T> collection, Predicate<T> checkPredicate) {
            for (int i = 0; i < collection.size(); i++){
                if (checkPredicate(collection.get(i)))
                    return collection.get(i);
            }
            return default(T);
        }

        public static T[] Filter<T>(this LinkedList<T> collection, Predicate<T> checkPredicate) {
            return (from i in Enumerable.Range(0, collection.size()).ToArray() where checkPredicate(collection.get(i)) select collection.get(i)).ToArray();
        }

        public static T[] Map<T>(this LinkedList<T> collection, Func<T, T> toApply) {
            return (from i in Enumerable.Range(0, collection.size()).ToArray() select toApply(collection.get(i))).ToArray();
        }
        ...
        public static double Reduce<T>(this LinkedList<T> collection, Func<T, double, double> funct){
            double counter = 0;
